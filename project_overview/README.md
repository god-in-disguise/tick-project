...
- Add a deliberate chart context control that switches between the truthful
  60-90 second live tape and a one-hour view, then returns to the exact live
  state. Both views must use the same market source, preserve real extrema, and
  avoid a blank frame or retrospective rescaling during the transition.
