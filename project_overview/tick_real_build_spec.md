...
| Other venues | Keep Aark, Aster, Lighter, Pacifica, GMTrade, and Ostium as research, probes, or shadow quote/health collectors. Aark passed live delegated round trips with an Aark-origin challenge and its live EIP-712 signer. TICK-origin challenges currently return `9999`, while the newly documented EIP-191 open returns `Invalid Signature`. Keep it disabled until Aark authorizes TICK's origins or partner key and confirms the canonical signer. Do not route consumer live flow to these venues yet. |
