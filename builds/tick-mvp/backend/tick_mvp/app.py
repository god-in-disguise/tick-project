...
from tick_mvp.auth import AuthError, UserSession, create_session_token, verify_session_token
from tick_mvp.config import get_settings
from tick_mvp.schemas import (
...
from tick_mvp.store import MemoryStore, StoreConflict, StoreNotFound
...
def create_app(store: MemoryStore | None = None) -> FastAPI:
    ...
    def open_trade(
        ...
        try:
            return _store(app).accept_open(_session(authorization, x_tick_user).user_id, body)
    ...
    def close_trade(
        ...
        try:
            return _store(app).accept_close(_session(authorization, x_tick_user).user_id, body)
        except StoreNotFound as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except StoreConflict as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
