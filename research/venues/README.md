...
| [gTrade / Gains](gtrade.md) | Documented + public measured | High-leverage wallet-native route | Arbitrum direct contract or delegated trading, oracle fulfillment | Forex 1000x, degen crypto 500x, BTC/ETH 200x, commodities 250x | Pair-specific; live backend exposes fee groups and minimum notional | Credits/revenue programs exist, but not core for TICK | Best next high-leverage live canary candidate |
...
1. Ask Aster whether the tested 1001x BNB/Arbitrum contracts are the active production trading path, and why direct opens revert as temporarily unavailable.
2. Ask GMTrade for private/order-keeper access; retest only if they can provide it.
3. Run a gTrade signed canary on Arbitrum, starting with BTCDEGEN/USD or BTC/USD.
4. Lighter signed latency/cost probe if API-key execution remains acceptable.
5. Pacifica only if 50x leverage is acceptable for a specific fallback route.
6. Keep Ostium as the live-tested wallet-native cross-asset connector and accounting benchmark.
